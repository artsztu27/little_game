...
export default function TicTacToe() {
    ...
    return (
        <div className="game-t flex content-center justify-center items-center mt-20 flex-col">
            <Board board={board} handleClick={handleOnClick} />
            {!isNoWinner && winner && (
                <p>{winner === "X" ? "You Win" : "AI Wins"}</p>
            )}
            {isNoWinner && <p> No one wins</p>}
            <button
                className="reset rounded-full border-slate-50 border-solid bg-slate-300 mt-9"
                type="button"
                onClick={() => restartGame()}>
                Start new Game
            </button>
